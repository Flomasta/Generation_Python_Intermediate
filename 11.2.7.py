...
for i in range(int(input())):
    ...
    # проверяем существует ли имя в списке
    if not d.get(name):
        d.setdefault(name, {item: int(amount)})
    else:
        # проверяем существует ли товар у пользователя
        if d[name].get(item):
            d[name][item] += int(amount)
        else:
            d[name].setdefault(item, int(amount))

for i in sorted(d):
    print(i + ':')
    for k, v in sorted(d[i].items()):
        print(k, v)
