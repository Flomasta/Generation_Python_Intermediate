with open('../txt_files_for_part_files_and_files_exam/func_file.txt', 'r', encoding='utf-8') as file:
    data = file.readlines()
    lst = []
    counter = 0
    for i in range(len(data)):
        # print(data[i])
        counter += data[i].count('def ')
        if 'def ' in data[i] and '#' not in data[i - 1]:
            lst.append(data[i].split()[1].split('(')[0])

    a = len(lst)
    if counter - len(lst) == counter:
