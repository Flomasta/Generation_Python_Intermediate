...
for i in range(n):
    for j in range(n):
        if abs(i - y) == abs(j - x) or i == y or j == x:
            board[i][j] = '*'
board[y][x] = 'Q'
...
# x, y = ('abcdefgh87654321'.index(c) % 8 for c in input())
# condition = lambda i, j: (j-i == x-y) + (j+i == x+y) + ((j == x) != (i == y))
# [print(*['.*Q'[condition(i, j)] for j in range(8)]) for i in range(8)]
